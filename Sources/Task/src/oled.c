...
//-----------------------------------------状态显示-----------------------------------------   
        ...
        OLED_BuffShowString(32,0,"状态显示",0);				//显示数据
        OLED_BuffShowString(0,2," ",1);
				OLED_BuffShowBigString(8,2,"这里是警告标识",1);                                       //显示数据
        OLED_BuffShowString(120,2," ",1);
				...
				OLED_BuffShow68Num02F(1,48,4,V,0);                                         //显示数据
        OLED_BuffShow68String(1,66,4,"V",0);
				if(general_mode==alarm_state)
				{
					if(flag)
					{
						OLED_BuffShowString(32,6,"电压异常",0);
						beep_1hz();
					}
					if(flag1)
					{
						OLED_BuffShowString(32,6,"有人经过\xfd",0);
						beep_4hz();
	//					beep_output();
					}
					if(flag2)
					{
						OLED_BuffShowString(32,6,"温度异常",0);
						beep_8hz();
					}
				}
...
// 其他备注: 写入的是缓存，需要电调用OLED_BuffShow才能显示出来
//========================================================================
void OLED_BuffShowChar(unsigned char x,unsigned char y,char asc, u8 mode)     //OLED显示一个8*16字符
{
    unsigned char j,k;
    for( j=0;j<2;j++ )
    {
...
void OLED_BuffShowChar68(unsigned char where,unsigned char x,unsigned char y,char asc, u8 mode)     //OLED显示一个6*8字符
{
    unsigned char j,k,n;
		if(where==1)
		{j=0;n=1;}
...
//========================================================================
void OLED_BuffShowGBK(unsigned char x,unsigned char y,char *gbk,u8 mode)     //OLED显示一个16*16汉字
{
    unsigned char i;
    unsigned char j,k;
    for( i=0;i<200;i++ )
